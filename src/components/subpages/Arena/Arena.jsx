...
export function Arena() {
  ...
  const handleFightClick = async () => {
    ...
    if (power1 === power2) {
      const updatedDraw = pokemonsInArena.map((p) => ({
        ...p,
        isInArena: false,
      }));

      setPokemonsData((prev) => {
        prev.map((p) => updatedDraw.find((pok) => pok.id === p.id) || p);
      });
    ...
    winnerId = power1 > power2 ? p1.id : p2.id;
    loserId = power1 > power2 ? p2.id : p1.id;
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        const winnerName = pokemonsInArena.find((p) => p.id === winnerId)?.name;
        enqueueSnackbar(`${winnerName} won the battle !`, {
          variant: "success",
        });
    ...
    <div className="mx-auto max-w-4xl p-4">
      ...
      <div className="grid grid-cols-1 items-center gap-4 md:grid-cols-3">
        <ArenaSlot
          ...
          onRemove={() => {
            const pokemon = pokemonsInArena[0];
            if (pokemon) {
              setPokemonsData((prev) =>
                prev.map((p) =>
                  p.id === pokemon.id
        ...
        <ArenaSlot
          ...
          onRemove={() => {
            const pokemon = pokemonsInArena[1];
            if (pokemon) {
              setPokemonsData((prev) =>
                prev.map((p) =>
                  p.id === pokemon.id
